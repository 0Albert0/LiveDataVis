...
def save_data(data_list):
    ...
    # 判断表是否存在，如果不存在则创建新表
    cursor.execute("SELECT count(*) FROM information_schema.TABLES WHERE table_name=%s", ('dy_1h',))
    table_exists = cursor.fetchone()[0]
    if not table_exists:
        cursor.execute("CREATE TABLE dy_1h("
        ...
        cursor.execute(
            "SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='dy_1h' AND COLUMN_NAME='gift_value'")
        column_type = cursor.fetchone()[0]
        if 'int' in column_type:
            cursor.execute("ALTER TABLE dy_1h MODIFY COLUMN gift_value VARCHAR(50)")

    # 删除所有数据
    cursor.execute("DELETE FROM dy_1h")

    for data in data_list:
        # 查询表结构，获取所有字段名
        cursor.execute("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'dy_1h'")
        columns = cursor.fetchall()
        column_names = [column[0] for column in columns]

        # 如果start_datetime字段不存在，则新建该字段，并在audience字段前面
        if 'start_datetime' not in column_names:
            cursor.execute("ALTER TABLE dy_1h ADD start_datetime VARCHAR(20) AFTER title")

        # 构造插入语句
        insert_sql = f"REPLACE INTO dy_1h (anchor_name, kind, link, room_id, title, start_datetime, audience, heat, pop, pop_peo, gift_value, gift_giver) VALUES ('{data['anchor_name']}', '{data['kind']}', '{data['link']}', '{data['room_id']}', '{data['title']}', '{data['start_datetime']}', '{data['audience']}', '{data['heat']}', '{data['pop']}', '{data['pop_peo']}', '{data['gift_value']}', '{data['gift_giver']}')"
