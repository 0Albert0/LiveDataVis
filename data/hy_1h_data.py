...
def get_data():
    ...
    # 打开目标网站
    url = 'https://bojianger.com/huya/list-intime.html'
    driver.get(url)
    ...
    # 点击指定位置
    elem = driver.find_element(By.XPATH, "/html/body/div[3]/div/div[2]/ul/li[1]/div/a[2]")
    elem.click()
    ...
    while True:  # 循环处理每一页的主播数据
        ...
        for row in rows:
            # 定位每个元素并获取其文本内容
            anchor_name    = row.find_element(By.XPATH, r'./div[1]/div[2]/h3/a').text
            kind           = row.find_element(By.XPATH, r'./div[1]/div[2]/p[1]').text
            link           = row.find_element(By.XPATH, './div[1]/div[2]/p[2]/a').get_attribute('href')
...
def save_data(data_list):
    # 连接数据库
    db = pymysql.connect(host='localhost', user='root', password='123456', database='livedata', charset='utf8mb4')
    ...
    # 判断表是否存在，如果不存在则创建新表
    cursor.execute("SELECT count(*) FROM information_schema.TABLES WHERE table_name=%s", ('hy_1h',))
    table_exists = cursor.fetchone()[0]
    if not table_exists:
        cursor.execute("CREATE TABLE hy_1h("
        ...
        cursor.execute(
            "SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='hy_1h' AND COLUMN_NAME='gift_value'")
        column_type = cursor.fetchone()[0]
        if 'int' in column_type:
            cursor.execute("ALTER TABLE hy_1h MODIFY COLUMN gift_value VARCHAR(50)")

    # 删除已存在的数据
    cursor.execute("DELETE FROM hy_1h")

    for data in data_list:
        # 构造插入语句的占位符和参数
        insert_sql = "REPLACE INTO hy_1h (anchor_name, kind, link, room_id, start_datetime, audience, heat, pop, pop_peo, gift_value, gift_giver) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
