...
def generate_hy_1h_html():
    ...
    sql = "SELECT kind, COUNT(room_id) as anchor_count, SUM(audience) as total_audience" \
          " FROM hy_1h GROUP BY kind ORDER BY SUM(heat) DESC LIMIT 10"
    cursor.execute(sql)
    ...
    # 查询总观看人数前10的直播类型
    sql = "SELECT kind, SUM(audience) as total_audience, SUM(pop) AS total_pop, SUM(pop_peo) AS total_pop_peo, SUM(gift_value) as total_gift_value, sum(gift_giver)as total_gift_giver FROM hy_1h GROUP BY kind ORDER BY total_audience DESC LIMIT 10"
    cursor.execute(sql)
    result = cursor.fetchall()
    kind_data = [i[0] for i in result]
    ...
    pie3 = (
        Pie()
            ...
            .set_series_opts(label_opts=opts.LabelOpts(formatter="{d}%", position="inside"))
            .set_global_opts(title_opts=opts.TitleOpts(title="弹幕数量-弹幕人数占比"), legend_opts=opts.LegendOpts(pos_left="18%"))
    )
    ...
    pie4 = (
        Pie()
            ...
            .set_series_opts(label_opts=opts.LabelOpts(formatter="{d}%", position="inside"))
            .set_global_opts(title_opts=opts.TitleOpts(title="礼物总值-送礼人数占比"), legend_opts=opts.LegendOpts(pos_left="18%"))
    )
    ...
    # 保存为HTML文件
    tab.render("./templates/hy_1h.html")
