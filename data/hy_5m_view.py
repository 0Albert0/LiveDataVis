...
def render_tab(host, port, user, password, database):
    # 在选项卡的标题上添加一个返回按钮
    back_button = """
    <button style='position: absolute; right: 10px; top: 10px' onclick=window.location.href='/huya/'>返回</button>
    """
    ...
    for kind in kind_lst:
        sql_list = [
            f"SELECT anchor_name, audience, heat FROM hy_5m WHERE kind = '{kind}' ORDER BY audience ",
            f"SELECT anchor_name, audience, heat FROM hy_5m WHERE kind = '{kind}' ORDER BY heat "
        ]
        for sql in sql_list:
            cursor.execute(sql)
    ...
    for kind in kind_lst:
        sql_list = [
            f"SELECT anchor_name, pop, pop_peo FROM hy_5m WHERE kind = '{kind}' ORDER BY pop ",
            f"SELECT anchor_name, pop, pop_peo FROM hy_5m WHERE kind = '{kind}' ORDER BY pop_peo  "
        ]
        for sql in sql_list:
            cursor.execute(sql)
            anchor_results = cursor.fetchall()
    ...
    for kind in kind_lst:
        sql_list = [
            f"SELECT anchor_name, gift_giver, gift_value FROM hy_5m WHERE kind = '{kind}' ORDER BY gift_giver ",
            f"SELECT anchor_name, gift_giver, gift_value FROM hy_5m WHERE kind = '{kind}' ORDER BY gift_value "
        ]
        for sql in sql_list:
            cursor.execute(sql)
            anchor_results = cursor.fetchall()
    ...
    tab1.render("./templates/hy_5m_tab1.html")
    tab2.render("./templates/hy_5m_tab2.html")
    tab3.render("./templates/hy_5m_tab3.html")
