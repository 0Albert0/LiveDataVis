...
def save_data(data_list):
    ...
    # 判断表是否存在，如果不存在则创建新表
    cursor.execute("SELECT count(*) FROM information_schema.TABLES WHERE table_name=%s", ('hy_5m',))
    table_exists = cursor.fetchone()[0]
    if not table_exists:
        cursor.execute("CREATE TABLE hy_5m("
        ...
        cursor.execute(
            "SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='hy_5m' AND COLUMN_NAME='gift_value'")
        column_type = cursor.fetchone()[0]
        if 'int' in column_type:
            cursor.execute("ALTER TABLE hy_5m MODIFY COLUMN gift_value VARCHAR(50)")

    # 删除已存在的数据
    cursor.execute("DELETE FROM hy_5m")

    for data in data_list:
        # 构造插入语句的占位符和参数
        insert_sql = "REPLACE INTO hy_5m (anchor_name, kind, link, room_id, start_datetime, audience, heat, pop, pop_peo, gift_value, gift_giver) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
